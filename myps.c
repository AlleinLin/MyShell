...
int getProcessInfo(char d_name[], struct ps_info *p1) // 读取进程信息，若读取成功，返回0，否则返回-1，并打印错误信息
{
    FILE *fd;
    char dir[20];
    struct stat infobuf;
...
psi *getAllProcess() // 遍历/proc目录所有进程目录,获取所有进程的信息，并存入进程链表中,返回链表的头指针
{
    DIR *dir_ptr;
    psi *head, *p1, *p2;
    char dir[] = "/proc/";
...
void printPS(struct ps_info *head) // 遍历进程链表，打印进程信息
{
    psi *list;
